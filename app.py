...
"""
KPO Dashboard – Streamlit додаток для моніторингу погоджень.
Дані завантажуються з Google Sheets, кеш оновлюється кожні 5 хвилин.
"""
...
KPO_COLORWAY = [
    KPO_CYAN, KPO_AMBER, KPO_GREEN, KPO_RED,
    KPO_PURPLE, KPO_ORANGE, KPO_BLUE, KPO_PINK
]
...
_kpo_dark_template = go.layout.Template(
    layout=go.Layout(
        ...
        xaxis=dict(gridcolor=KPO_BORDER, zerolinecolor=KPO_BORDER,
                   linecolor=KPO_BORDER),
        yaxis=dict(gridcolor=KPO_BORDER, zerolinecolor=KPO_BORDER,
                   linecolor=KPO_BORDER),
        legend=dict(bgcolor="rgba(0,0,0,0)"),
        hoverlabel=dict(bgcolor=KPO_CARD_BG, font_color=KPO_TEXT,
...
try:
    ...
    st.error("Не знайдено SPREADSHEET_ID у secrets. "
...
def load_data():
        ...
        for header_row, month, year in month_rows:
            ...
            try:
                ...
                for r in range(header_row + 1,
                               min(header_row + TOTAL_ROW_SEARCH_RANGE,
                                   len(values))):
                    if len(values[r]) > 0 and normalize_operation(
                            values[r][0]) == "Тотал":
                        total_row_idx = r
                        break

            if total_row_idx is not None:
                sum_true = as_number(values[total_row_idx][1]) \
                    if len(values[total_row_idx]) > 1 else 0
                sum_false = as_number(values[total_row_idx][2]) \
                    if len(values[total_row_idx]) > 2 else 0
                ...
                per_op_tf_found = set()
                for r in range(total_row_idx + 1,
                               min(total_row_idx + 1 +
                                   PER_OP_TF_SEARCH_RANGE, len(values))):
                    ...
                    if op_name not in OPERATIONS or op_name in per_op_tf_found:
                        continue
                    op_sum_true = as_number(values[r][1]) \
                        if len(values[r]) > 1 else 0
                    op_sum_false = as_number(values[r][2]) \
                        if len(values[r]) > 2 else 0
                    op_true_false.append({
                        "month": month_key,
                        "operation": op_name,
                ...
                missing_tf_ops = [op for op in OPERATIONS
                                  if op not in per_op_tf_found]
                if missing_tf_ops:
                    warnings.append(
                        f"⚠️ Аркуш «{sheet_name}», {month_label}: "
                        f"не знайдено TRUE/FALSE дані для операцій: "
                        f"{', '.join(missing_tf_ops)}."
                    )
            else:
                warnings.append(
                    f"⚠️ Аркуш «{sheet_name}», {month_label}: "
                    "не знайдено рядок «Тотал»."
                )

            detail_start = None
            for r in range(header_row + 1,
                           min(header_row + DETAIL_SEARCH_RANGE, len(values))):
                if len(values[r]) > 3 and normalize_operation(
                        values[r][3]) in OPERATIONS:
                    detail_start = r
                    break
            ...
            if detail_start is None:
                warnings.append(
                    f"⚠️ Аркуш «{sheet_name}», {month_label}: "
                    "не знайдено таблицю деталізації."
                )
                continue
            ...
            for r in range(detail_start, len(values)):
                ...
                for day_idx in range(days):
                    ...
                    date = pd.Timestamp(year=year, month=month,
                                        day=day_idx + 1)
                    records.append({
                        "date": date,
                        "operation": operation,
    ...
    tf_df = pd.DataFrame(op_true_false).drop_duplicates(subset=["month",
                                                                "operation"])
...
def detect_anomalies(df, window=14, threshold=3.0):
    ...
    for month, group in daily.groupby("month_key"):
        ...
        group["rolling_median"] = group["value"].rolling(
            window=window, min_periods=1, center=True
        ).median()
        group["rolling_mad"] = group["value"].rolling(
            window=window, min_periods=1, center=True
        ).apply(
            lambda x: np.median(np.abs(x - np.median(x)))
            if len(x) > 1 else 0
        )
        group["z_score"] = (group["value"] - group["rolling_median"]) / (
            group["rolling_mad"] * 1.4826).replace(0, 1)
...
def forecast_scenarios(df, current_month):
    ...
    today = now_kyiv()
    days_passed = (today - pd.Timestamp(year=today.year,
                                        month=today.month,
                                        day=1)).days + 1

    if (today.month != pd.Period(current_month).month or
            today.year != pd.Period(current_month).year):
        return None, None
    ...
    if prev_period_str in df["month"].unique():
        ...
        if (not prev_fact.empty() and prev_fact["value"].sum() > 0 and
                prev_fact["value"].sum() >= 10):
...
def main():
    ...
    st.caption(
        ...
        "Кеш оновлюється кожні 5 хвилин. Час — за Києвом."
    )
    ...
    if period_mode == "За місяцями":
        selected_years = st.sidebar.multiselect(
            "Рік", options=years, default=default_years
        )
        available_months = df[df["year"].isin(selected_years)]["month"] \
        ...
        if isinstance(date_range_input, tuple) and len(date_range_input) == 2:
            custom_range = (pd.Timestamp(date_range_input[0]),
                            pd.Timestamp(date_range_input[1]))
        else:
            single = date_range_input[0] if isinstance(date_range_input,
                                                       tuple) else date_range_input
        ...
        selected_months = sorted({d.strftime("%Y-%m")
                                 for d in pd.date_range(custom_range[0],
                                                        custom_range[1],
                                                        freq="D")})
    ...
    if operation_mode == "Тотал":
        ...
        selected_operations = st.sidebar.multiselect(
            "Операції", options=all_ops, default=default_ops
        )
    ...
    smooth_enabled = st.sidebar.checkbox(
        "Згладжування динаміки (ковзне середнє)", value=False
    )
    smooth_window = 7
    if smooth_enabled:
        smooth_window = st.sidebar.selectbox(
            "Вікно згладжування (дні)", [3, 5, 7, 14], index=2
        )
    ...
    if filtered.empty:
        st.warning(
            "За вибраними фільтрами даних немає "
            "(можливо, ще немає даних за цей період)."
        )
        st.stop()
    ...
    if filtered_stats.empty:
        st.info(
            "Дані ще не внесені для жодного дня у вибраному періоді — "
            "статистика недоступна, показано лише графіки."
        )
    ...
    daily_avg_weekday = filtered_stats[weekday_mask].groupby(
        "date"
    )["value"].sum().mean() if weekday_mask.any() else None
    daily_avg_weekend = filtered_stats[weekend_mask].groupby(
        "date"
    )["value"].sum().mean() if weekend_mask.any() else None
    ...
    if period_mode == "За місяцями":
        ...
        if full_months:
            tf_filtered = df[
                ...
                (df["operation"].isin(selected_operations) if
                 operation_mode != "Тотал" else df["operation"] == "Тотал")
            ][["month", "operation", "sum_true", "sum_false"]].drop_duplicates()
        else:
    ...
    if not tf_filtered.empty:
        ...
        approval_rate_val = (sum_true_total / total_ratio * 100) \
            if total_ratio > 0 else 0
        approval_rate_str = f"{approval_rate_val:.1f}%" \
            if total_ratio > 0 else "—"
    ...
    if period_mode == "За місяцями":
        period_mask = df["year"].isin(selected_years) & df["month"].isin(
            selected_months)
    else:
        if full_months:
    ...
    approval_by_op = pd.DataFrame(
        columns=["operation", "sum_true", "sum_false", "total", "approval_rate"]
    )
    if not tf_by_op_all.empty:
        approval_by_op = tf_by_op_all.groupby(
            "operation", as_index=False
        )[["sum_true", "sum_false"]].sum()
        approval_by_op["total"] = approval_by_op["sum_true"] + approval_by_op[
            "sum_false"]
        approval_by_op = approval_by_op[approval_by_op["total"] > 0].copy()
        approval_by_op["approval_rate"] = (
            approval_by_op["sum_true"] / approval_by_op["total"] * 100
        ).round(1)
        approval_by_op = approval_by_op.sort_values("approval_rate",
                                                    ascending=False)

    # --- Порівняння ---
    comparison_parts = []
    if (period_mode == "За місяцями" and len(selected_months) == 1 and
            operation_mode == "Тотал"):
        ...
        if current_period.end_time <= today_comp:
            ...
            delta_prev = ((cur_sum - prev_sum) / prev_sum * 100) \
                if prev_sum > 0 else None
        else:
            day_limit = today_comp.day
            cur_sum = filtered_stats[filtered_stats["date"].dt.day <=
                                     day_limit]["value"].sum()
        ...
        if has_prev_year:
            if current_period.end_time <= today_comp:
                ...
                delta_year = ((cur_sum - prev_year_sum) / prev_year_sum * 100) \
                    if prev_year_sum > 0 else None
            else:
                day_limit = today_comp.day
                cur_sum = filtered_stats[filtered_stats["date"].dt.day <=
                                         day_limit]["value"].sum()
    ...
    def forecast_cards(title, forecast, help_base=None, help_min=None,
                       help_max=None):
        if forecast is None:
            return
        ...
        with col1:
            st.markdown(
                custom_metric(f"{title} (базовий)",
                              f"{forecast['base']:,.0f}", help_base),
                unsafe_allow_html=True
            )
        with col2:
            st.markdown(
                custom_metric(f"{title} (консервативний)",
                              f"{forecast['min']:,.0f}", help_min),
                unsafe_allow_html=True
            )
        with col3:
            st.markdown(
                custom_metric(f"{title} (оптимістичний)",
                              f"{forecast['max']:,.0f}", help_max),
                unsafe_allow_html=True
            )
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "📊 Overview", "📈 Динаміка", "🧩 Операції",
        "📅 Навантаження", "🆚 Порівняння періодів"
    ])
    ...
    with tab1:
        ...
        with col1:
            st.markdown(
                custom_metric(
                    "Всього",
                    f"{total_value:,.0f}",
                    "Загальна кількість операцій за вибраний період "
                    "(включно з календарними днями без внесених даних)"
                ),
                unsafe_allow_html=True
            )
        with col2:
            st.markdown(
                custom_metric(
                    "Середнє за день",
                    f"{daily_avg:.0f}",
                    "Сумарна кількість поділена на кількість днів, "
                    "за які реально внесені дані "
                    "(порожні клітинки не враховуються як нулі)"
                ),
                unsafe_allow_html=True
            )
        with col3:
            avg_weekday_str = f"{daily_avg_weekday:.0f}" \
                if daily_avg_weekday is not None and not pd.isna(daily_avg_weekday) \
                else "—"
            st.markdown(
                custom_metric(
                    "Середнє за будні",
                    avg_weekday_str,
                    "Середня кількість операцій у будні "
                    "(лише дні з внесеними даними)"
                ),
                unsafe_allow_html=True
            )
        with col4:
            avg_weekend_str = f"{daily_avg_weekend:.0f}" \
                if daily_avg_weekend is not None and not pd.isna(daily_avg_weekend) \
                else "—"
            st.markdown(
                custom_metric(
                    "Середнє за вихідні",
                    avg_weekend_str,
                    "Середня кількість операцій у вихідні "
                    "(лише дні з внесеними даними)"
                ),
                unsafe_allow_html=True
            )
        with col5:
            ...
            st.markdown(
                custom_metric(
                    "Пік за день",
                    peak_display,
                    "Найбільша кількість операцій за один день "
                    "(лише дні з внесеними даними). У дужках – дата піку."
                ),
                unsafe_allow_html=True
            )
        with col6:
            st.markdown(
                custom_metric(
                    ...
                    approval_rate_str,
                    f"Частка TRUE (погоджено) від TRUE+FALSE. "
                    f"{'Доступно лише для повних календарних місяців.' if period_mode != 'За місяцями' else ''} "
                    f"🟢 ≥{APPROVAL_GOOD_THRESHOLD}% "
                    f"🟡 {APPROVAL_WARN_THRESHOLD}-{APPROVAL_GOOD_THRESHOLD}% "
                    f"🔴 <{APPROVAL_WARN_THRESHOLD}%",
                    color=approval_rate_color(
                        approval_rate_val if approval_rate_available else None
                    ),
                ),
                unsafe_allow_html=True
            )
        ...
        with col7:
            st.markdown(
                custom_metric(
                    "Пік / середнє",
                    f"{peak_avg_ratio:.2f}×",
                    "У скільки разів пік перевищує середнє"
                ),
                unsafe_allow_html=True
            )
        with col8:
            st.markdown(
                custom_metric(
                    ...
                    "Коефіцієнт варіації (лише дні з внесеними даними). "
                    "🟢 <15% 🟡 15-30% 🔴 >30%",
        ...
        with col9:
            if busiest_weekday:
                ...
                help_txt = "День тижня з найвищим середнім навантаженням " \
                           "(лише дні з внесеними даними)"
            else:
                val = "—"
                help_txt = None
            st.markdown(
                custom_metric("Найактивніший день", val, help_txt),
                unsafe_allow_html=True
            )
        with col10:
            if busiest_op:
                display_name = busiest_op if len(busiest_op) <= 12 \
                    else busiest_op[:10] + "…"
                ...
                help_txt = None
            st.markdown(
                custom_metric("Найактивніша операція", val, help_txt),
                unsafe_allow_html=True
            )
        with col11:
            if (period_mode == "За місяцями" and len(selected_months) == 1 and
                ...
                if comparison_text != "—":
                    ...
                    st.markdown("<p class='comparison-text'>—</p>",
                                unsafe_allow_html=True)
            else:
                st.markdown(
                    custom_metric(
                        "Порівняння",
                        "—",
                        "Доступно лише для одного місяця в режимі "
                        "'За місяцями' + 'Тотал'"
                    ),
                    unsafe_allow_html=True
                )
        ...
        if approval_rate_available:
            if approval_rate_val >= APPROVAL_GOOD_THRESHOLD:
                insights.append(
                    ...
                    f"вище порогу {APPROVAL_GOOD_THRESHOLD}%, хороший показник."
                )
            elif approval_rate_val >= APPROVAL_WARN_THRESHOLD:
                insights.append(
                    f"🟡 Коефіцієнт погоджень **{approval_rate_str}** — "
                    f"у середній зоні ({APPROVAL_WARN_THRESHOLD}-{APPROVAL_GOOD_THRESHOLD}%), "
                    "варто стежити за динамікою."
                )
            else:
                insights.append(
                    f"🔴 Коефіцієнт погоджень **{approval_rate_str}** — "
                    f"нижче {APPROVAL_WARN_THRESHOLD}%, потребує уваги."
                )
        else:
            insights.append(
                "ℹ️ Коефіцієнт погоджень недоступний для вибраного діапазону "
                "(потрібні повні календарні місяці)."
            )
        ...
        if not approval_by_op.empty and len(approval_by_op) > 1:
            ...
            if best_row["operation"] != worst_row["operation"]:
                insights.append(
                    f"🧩 Найкращий % погоджень — **{best_row['operation']}** "
        ...
        if busiest_weekday:
            day_ua = WEEKDAY_UA.get(busiest_weekday, busiest_weekday)
            insights.append(
                f"📅 Найбільше навантаження припадає на **{day_ua}** — "
                f"в середньому {busiest_weekday_val:.0f} операцій/день."
            )

        if busiest_op:
            insights.append(
                f"📈 Найактивніша операція за обсягом — **{busiest_op}** "
                f"({busiest_op_val:,.0f} за період)."
            )

        if peak_avg_ratio >= 2:
            insights.append(
                f"⚠️ Пік у **{peak_avg_ratio:.1f}×** перевищує середнє — "
                "можливі різкі сплески навантаження, варто мати запас потужності."
            )

        if period_mode == "За місяцями" and comparison_text != "—":
            insights.append(
                f"🔄 Порівняння з попередніми періодами: {comparison_text}."
            )
        ...
        # --- Прогнози ---
        if period_mode != "За місяцями":
            st.info("📊 Прогнози доступні лише в режимі 'За місяцями' "
                    "з одним обраним місяцем.")
        elif len(selected_months) != 1:
            st.info("📊 Прогнози доступні лише для одного обраного місяця "
                    "(виберіть один місяць у сайдбарі).")
        else:
            ...
            forecast_target = st.selectbox(
                ...
                index=0,
                help="Прогноз можна будувати для Тоталу або окремої операції "
                     "— незалежно від 'Режиму показу' графіків нижче.",
            )
            stat_forecast, season_forecast = forecast_scenarios(
                df[df["operation"] == forecast_target],
                selected_months[0]
            )
            if stat_forecast or season_forecast:
                ...
                if stat_forecast:
                    st.markdown(
                        "**📈 Статистичний прогноз обсягу** "
                        "(на основі середнього та варіативності минулих днів)"
                    )
                    forecast_cards(
                        "Стат.",
                        stat_forecast,
                ...
                if season_forecast:
                    st.markdown(
                        "**📅 Сезонний прогноз обсягу** "
                        "(на основі динаміки аналогічного періоду минулого року)"
                    )
                    with st.expander("🔍 Деталі розрахунку сезонного прогнозу"):
                        ...
                        st.write(
                            f"**Сума за аналогічний період минулого року:** "
                            f"{season_forecast['prev_fact_sum']:,.0f}"
                        )
                        st.write(
                        ...
                        st.write(
                            f"**Сума за залишок місяця (минулий рік):** "
                            f"{season_forecast['prev_remaining_sum']:,.0f}"
                        )
                        st.write(
                            f"**Прогноз на залишок (з урахуванням коефіцієнта):** "
                            f"{season_forecast['forecast_remaining']:,.0f}"
                        )
                        st.write(
                            f"**Загальний прогноз (факт + прогноз на залишок):** "
                            f"{season_forecast['base']:,.0f}"
                        )
                    forecast_cards(
                        "Сезон.",
                        season_forecast,
                ...
                st.info(
                    "Прогноз недоступний: немає фактичних даних за поточний місяць "
                    "для цієї операції."
                )
        ...
        if operation_mode == "Тотал":
            ...
            if smooth_enabled:
                daily["value_smooth"] = daily["value"].rolling(
            ...
            fig_overview = px.line(
                filtered, x="date", y="value", color="operation", markers=True,
                labels={"date": "Дата", "value": "Кількість",
                        "operation": "Операція"}
            )
            fig_overview.update_xaxes(tickformat="%d.%m", title_text="Дата")
    ...
    with tab2:
        ...
        if operation_mode == "Тотал":
            ...
            fig_daily_detailed.update_xaxes(tickformat="%d.%m",
                                            title_text="Дата")
            if smooth_enabled:
                daily["value_smooth"] = daily["value"].rolling(
                    window=smooth_window, min_periods=1, center=True
        ...
        if operation_mode == "Тотал":
            ...
            yoy_monthly = yoy_data.groupby(["year", "month"])["value"] \
                .sum().reset_index()
            yoy_monthly["month_num"] = yoy_monthly["month"].apply(
                lambda x: pd.Period(x).month
            )
            yoy_monthly["month_label"] = yoy_monthly["month"].apply(
                lambda x: pd.Period(x).strftime("%b")
            )
            yoy_monthly = yoy_monthly.sort_values(["year", "month_num"])
            month_axis_order = yoy_monthly.drop_duplicates("month_num") \
                .sort_values("month_num")["month_label"].tolist()
            ...
            if len(selected_years) >= 2:
                ...
                if len(years_sorted) >= 2:
                    ...
                    y1_data = yoy_monthly[yoy_monthly["year"] == y1].set_index(
                        "month_label"
                    )["value"]
                    y2_data = yoy_monthly[yoy_monthly["year"] == y2].set_index(
                        "month_label"
                    )["value"]
                    compare_df = pd.DataFrame(
                        {str(y1): y1_data, str(y2): y2_data}
                    ).fillna(0)
                    compare_df["Різниця"] = compare_df[str(y2)] - compare_df[
                        str(y1)]
                    compare_df["%"] = (compare_df["Різниця"] / compare_df[
                        str(y1)] * 100).fillna(0)
                    compare_df["%"] = compare_df["%"].apply(
                        lambda x: f"{x:+.1f}%"
                    )
                    st.dataframe(compare_df, use_container_width=True)

            st.subheader("📈 Накопичувальна сума за період")
            cumsum = filtered.groupby("date")["value"].sum().sort_index() \
                .cumsum().reset_index()
            cumsum.columns = ["date", "cumulative"]
            fig_cum = px.line(
                cumsum, x="date", y="cumulative", markers=True,
            ...
            if not anomalies.empty:
                ...
                if not anomaly_points.empty:
                    ...
                    anomaly_points["date_str"] = anomaly_points["date"].dt.strftime(
                        "%d.%m.%Y"
                    )
                    anomaly_points["deviation"] = (
                        (anomaly_points["value"] - anomaly_points["rolling_median"]) /
                        anomaly_points["rolling_median"] * 100
                    ).round(1)
                    anomaly_points["type"] = anomaly_points["deviation"].apply(
                        lambda x: "🔴 Високий" if x > 10
                        else "🔵 Низький" if x < -10 else "🟡 Помірний"
                    )
                    anomaly_points = anomaly_points.sort_values("date",
                                                                ascending=False)
                    st.dataframe(
                        anomaly_points[[
                            "date_str", "value", "rolling_median",
                            "deviation", "type", "z_score"
                        ]],
                        column_config={
                            "date_str": "Дата",
                            "value": "Значення",
                            ...
                            "z_score": st.column_config.NumberColumn(
                                "Z-score (MAD)",
                                help="Кількість MAD від медіани. "
                                     "Значення >3.0 вважається аномалією."
                            )
                        },
    ...
    with tab3:
        ...
        if not approval_by_op.empty:
            ...
            total_rate = (total_true / total_ratio_ops * 100) \
                if total_ratio_ops > 0 else None

            approval_by_op_display = approval_by_op.copy()
            approval_by_op_display["tier"] = approval_by_op_display[
                "approval_rate"
            ...
            fig_approval = px.bar(
                ...
                category_orders={
                    "operation": approval_by_op_display["operation"].tolist()
                },
            )
            ...
            with st.expander("📋 Таблиця по операціях", expanded=False):
                st.dataframe(
                    approval_by_op[[
                        "operation", "sum_true", "sum_false", "total",
                        "approval_rate"
                    ]].rename(columns={
                        "operation": "Операція",
        ...
        st.subheader("🌡️ Теплова карта коефіцієнта погоджень "
                     "(Операція × Місяць)")
        tf_heat = df[period_mask][[
            "month", "operation", "sum_true", "sum_false"
        ]].drop_duplicates()
        if not tf_heat.empty:
            ...
            if not tf_heat.empty:
                ...
                tf_heat["month_label"] = tf_heat["month"].apply(
                    lambda x: pd.Period(x).strftime("%m.%Y")
                )
                heat_rate_pivot = tf_heat.pivot_table(
                    index="month_label",
                    columns="operation",
                    values="rate",
                    aggfunc="mean"
                ...
                sorted_month_labels = sorted(heat_rate_pivot.index,
                                             key=_sort_month_label)
                ...
                heatmap_height = max(
                    min_height,
                    min(max_height, len(heat_rate_pivot.index) * row_height)
                )
                fig_approval_heat.update_layout(
                    height=heatmap_height,
                    margin=dict(l=10, r=10, t=20, b=10)
                )
                font_size = 12 if len(heat_rate_pivot.index) <= 12 else (
                    10 if len(heat_rate_pivot.index) <= 24 else 8
                )
                fig_approval_heat.update_traces(textfont=dict(size=font_size))
                st.plotly_chart(fig_approval_heat, use_container_width=True)
                st.caption(
                    "🔴 <70% 🟡 70-85% 🟢 >85% — кольорова шкала неперервна."
                )
            else:
        ...
        if not ops_data.empty:
            ...
            ops_structure = ops_data.groupby("operation")["value"].sum() \
                .reset_index().sort_values("value", ascending=False)
            ops_structure["percent"] = (ops_structure["value"] /
                                        ops_structure["value"].sum() * 100).round(1)
            ...
            ops_monthly = ops_data.groupby(["month", "operation"])["value"] \
                .sum().reset_index()
            ops_monthly["month_label"] = ops_monthly["month"].apply(
                lambda x: pd.Period(x).strftime("%m.%Y")
            )
            fig_stacked = px.bar(
                ops_monthly,
            ...
            pareto_data = ops_structure.copy().sort_values("value",
                                                           ascending=False)
            ...
            if operation_mode != "Тотал" and len(selected_operations) > 1:
                ...
                fig_compare_ops.update_xaxes(tickformat="%d.%m",
                                             title_text="Дата")
    ...
    with tab4:
        ...
        weekday_avg = daily_sum.groupby("weekday_ua")["value"].mean() \
            .reindex(WEEKDAY_ORDER_UA).reset_index()
        ...
        week_data = filtered.assign(
            period_type=filtered["is_weekend"].map({False: "Будні",
                                                    True: "Вихідні"})
        ).groupby(["month", "period_type"], as_index=False)["value"].sum()
        week_data["month_total"] = week_data.groupby("month")["value"] \
            .transform("sum")
        week_data["percent"] = (week_data["value"] / week_data["month_total"] *
                                100).round(1)
        ...
        heat_data = daily_heat.groupby(["month_label", "weekday_ua"])["value"] \
            .mean().reset_index()
        heat_pivot = heat_data.pivot(index="month_label", columns="weekday_ua",
        ...
        heatmap_height = max(
            min_height,
            min(max_height, len(heat_pivot.index) * row_height)
        )
        ...
        fig_heatmap = px.imshow(
            ...
            text_auto=".1f",
            aspect="auto",
            labels=dict(x="День тижня", y="Місяць",
                        color="Середня кількість"),
            color_continuous_scale=KPO_HEAT_SCALE
        )
        fig_heatmap.update_layout(
            height=heatmap_height, margin=dict(l=10, r=10, t=20, b=10)
        )
        font_size = 12 if len(heat_pivot.index) <= 12 else (
            10 if len(heat_pivot.index) <= 24 else 8
        )
        ...
        col1, col2, col3 = st.columns(3)
        col1.markdown(
            custom_metric("Середнє за день", f"{daily_avg:.0f}"
                          if daily_avg > 0 else "—"),
            unsafe_allow_html=True
        )
        col2.markdown(
            custom_metric("Стандартне відхилення", f"{std:.1f}"
                          if std > 0 else "—"),
            unsafe_allow_html=True
        )
        col3.markdown(
            custom_metric("Коефіцієнт варіації (CV)", f"{cv:.1f}%"
                          if cv > 0 else "—"),
            unsafe_allow_html=True
        )
        ...
        if len(daily_totals) >= 3:
            ...
            mean_weekday = daily_totals.loc[weekday_mask, "value"].mean() \
                if weekday_mask.any() else None
            mean_weekend = daily_totals.loc[weekend_mask, "value"].mean() \
                if weekend_mask.any() else None

            daily_totals["dev_weekday"] = None
            daily_totals.loc[weekday_mask, "dev_weekday"] = (
                (daily_totals.loc[weekday_mask, "value"] - mean_weekday) /
                mean_weekday * 100
            ...
            daily_totals.loc[weekend_mask, "dev_weekend"] = (
                (daily_totals.loc[weekend_mask, "value"] - mean_weekend) /
                mean_weekend * 100
            ...
            dev_wd = daily_totals.loc[weekday_mask, "dev_weekday"].dropna().values \
                if weekday_mask.any() else np.array([])
            if len(dev_wd) > 1:
                dev_data.append(dev_wd)
                group_names.append("Будні")
            dev_we = daily_totals.loc[weekend_mask, "dev_weekend"].dropna().values \
                if weekend_mask.any() else np.array([])
            if len(dev_we) > 1:
            ...
            if dev_data:
                ...
                colors = {"Всі дні": KPO_CYAN, "Будні": KPO_GREEN,
                          "Вихідні": KPO_AMBER}
                max_density = 0
                for group_name, data in zip(group_names, dev_data):
                    if len(data) > 1:
                        ...
                        fig_density.add_trace(go.Scatter(
                            ...
                            mode='lines',
                            name=group_name,
                            line=dict(color=colors.get(group_name, "gray"),
                                      width=2.5),
                            fill='none'
                        ))
        ...
        st.markdown(
            custom_metric("Пік / середнє", f"{peak_avg_ratio:.2f}×"
                          if peak_avg_ratio > 0 else "—"),
            unsafe_allow_html=True
        )
    ...
    with tab5:
        ...
        st.caption(
            "Незалежно від фільтрів у сайдбарі — оберіть два будь-які "
            "діапазони дат для порівняння."
        )
        ...
        def _normalize_range(range_input):
            if isinstance(range_input, tuple) and len(range_input) == 2:
                start, end = pd.Timestamp(range_input[0]), pd.Timestamp(
                    range_input[1])
            else:
                single = range_input[0] if isinstance(range_input,
                                                      tuple) else range_input
                start = end = pd.Timestamp(single)
            if start > end:
                start, end = end, start
            return start, end

        if not (isinstance(range_a_input, tuple) and len(range_a_input) == 2 and
                isinstance(range_b_input, tuple) and len(range_b_input) == 2):
            st.info("Оберіть повний діапазон (початкову і кінцеву дату) "
                    "для обох періодів.")
        else:
            ...
            def build_period_metrics(date_range, ops):
                ...
                if full_months:
                    tf = scoped[scoped["month"].isin(full_months)][[
                        "month", "operation", "sum_true", "sum_false"
                    ]].drop_duplicates()
                    s_true = tf["sum_true"].sum()
                    s_false = tf["sum_false"].sum()
                    rate = (s_true / (s_true + s_false) * 100) \
                        if (s_true + s_false) > 0 else None
                else:
                    rate = None
            ...
            if metrics_a is None or metrics_b is None:
                ...
                rows = [
                    ...
                    {"Метрика": "Коефіцієнт погоджень, %",
                     "A": f"{metrics_a['rate']:.1f}%" if metrics_a["rate"] is not None
                     else "— (немає повних місяців)",
                     "B": f"{metrics_b['rate']:.1f}%" if metrics_b["rate"] is not None
                     else "— (немає повних місяців)",
                     "Δ": (f"{metrics_b['rate'] - metrics_a['rate']:+.1f} п.п."
                ...
                }).melt(id_vars="Метрика", var_name="Період",
                        value_name="Значення")
        ...
        st.caption(
            "Джерело: Google Sheets • Оновлення даних: до 5 хвилин "
            "після зміни таблиці • Час: Europe/Kyiv."
        )
